...
- 行は 3 段だけ: タイトル（判断依頼の title / `path:L10–12` / エージェントの状態）、補足（設問数や返信の冒頭）、メタ（種別 · リポジトリ › ブランチ · エージェント · 経過時間を 1 行に。リポジトリ名はサイドバーと同じ衝突回避、ブランチは main が無色・linked worktree は色付き chip）。右端は配達 chip（問題があるときだけ）と遷移の矢印。状態 chip や種別ラベルは行に置かない。
