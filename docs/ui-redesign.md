...
- 編集: ビューアヘッダーの「編集」トグル（`editable: false` は理由付きで無効化、html はプレビュー中は無効 — WYSIWYG 編集を持たない）で `CodeFileView` を編集可能にする。編集中は行選択・質問アノテーションを止める（行がずれるため）。下書きはタブ・worktree ごとに保持し、タブに dirty マーク、離脱・タブクローズ・トグル OFF はいずれも保存していない変更があれば確認する。保存は「保存」ボタンか `Cmd+S`/`Ctrl+S` で明示的に行い（自動保存はしない）、ディスク側の改行コード（CRLF）に揃えてから送る。409（衝突）とディスク上の変更検出は同じバナーで「上書き保存」/「破棄して再読込」を選べる。ディスク上の変更検出は `repo-changed`（git status のポーリング/イベント）頼みだと、既に `M`（変更あり）なファイルへの再書き込みは検出できないため、編集中だけ 3 秒間隔のポーリングを別に回す。
  - Markdown はプレビュー（Tiptap の WYSIWYG）のままでも編集できる。Tiptap の markdown シリアライズは書式を正規化する（リスト記号の統一、表の桁揃えなど）ため、そのまま保存すると触っていない行まで diff に乗る — `markdownMerge.ts` の対応付け＋領域吸収アルゴリズムが、ディスク上の原文を基準に実際に編集した段落だけを書き換える。正規化された段落自体を編集した場合はその段落だけ正規化後の書式になり、ビューア下部に注記が出る。YAML front matter（`---` 区切り）は Tiptap（`@tiptap/markdown`、upstream issue #7152 は対応予定なしでクローズ）の外で扱う — `frontMatter.ts` の `splitFrontMatter`/`joinFrontMatter` で本文と分離し、本文だけを Tiptap に渡す。front matter はキー / 値のテーブルで表示・編集する（YAML は解析せず行単位、`frontMatterEntries.ts`）。
