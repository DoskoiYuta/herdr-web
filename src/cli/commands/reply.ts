...
/**
 * `hw review reply <id> <text...>` — `text` is the remaining positionals
 * joined with a space, or `-` alone to read the body from stdin.
 */
export async function reviewReplyCommand(
  argv: string[],
  deps: CommandDeps,
  ...
  if (!id || textWords.length === 0) {
    return usageError(
      "usage: hw review reply <id> <text...> (or `hw review reply <id> -` to read stdin)",
    );
  }
  ...
  const result = await deps.client.replyToReview(id, body, ctx.sessionKey);
