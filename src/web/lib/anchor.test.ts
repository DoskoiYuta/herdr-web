...
test("buildAnchor: middle line gets up to 3 lines of before/after context", async () => {
  ...
  const anchor = await buildAnchor(lines, 3, 3, "new");
  expect(anchor.side).toBe("new");
  expect(anchor.lines).toEqual(["four"]);
...
test("buildAnchor: first line has no before context", async () => {
  const anchor = await buildAnchor(["a", "b", "c"], 0, 0, "old");
  expect(anchor.before).toEqual([]);
  expect(anchor.lines).toEqual(["a"]);
});

test("buildAnchor: a multi-line selection captures every selected line", async () => {
  const lines = ["one", "two", "three", "four", "five"];
  const anchor = await buildAnchor(lines, 1, 3, "new");
  expect(anchor.lines).toEqual(["two", "three", "four"]);
  expect(anchor.before).toEqual(["one"]);
  expect(anchor.after).toEqual(["five"]);
});

test("buildAnchor: hash is stable for the same input", async () => {
  const lines = ["x", "y", "z"];
  const a = await buildAnchor(lines, 1, 1, "new");
  const b = await buildAnchor(lines, 1, 1, "new");
  expect(a.hash).toBe(b.hash);
});

test("buildAnchor: normalizes trailing whitespace before hashing", async () => {
  const withTrailing = await buildAnchor(["foo  ", "bar\t", "baz"], 1, 1, "new");
  const withoutTrailing = await buildAnchor(["foo", "bar", "baz"], 1, 1, "new");
